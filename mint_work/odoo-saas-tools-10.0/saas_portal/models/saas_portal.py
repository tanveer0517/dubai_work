...
class SaasPortalServer(models.Model):
    ...
    @api.multi
    def action_syn_own_servers(self):
        for r in self:
            r.action_sync_server()
        return True
    ...
    def action_sync_server(self, updating_client_ID=None):
        ...
        try:
            ...
            # client_not_found = data[0].get('client_not_found')
            # client_uuid = data[0].get('client_uuid')
            # if client_not_found:
            #     client_rec = self.env['saas_portal.client'].sudo().search(
            #         [('client_id','=',client_uuid)], limit=1)
            #     print "\\nn\n not found -------",client_rec
            #     user_id = self.env['res.users'].sudo().search(
            #         [('partner_id','=',client_rec.partner_id.id)], limit=1
            #     )
            #     if client_rec.trial:
            #         trial = True
            #     else:
            ...
            #     new_client_id = client_rec.plan_id.create_new_database(
            #         dbname = client_rec.name,
            #         client_id = client_uuid,
            #         partner_id = client_rec.partner_id.id,
            #         user_id = user_id.id,
            #         notify_user = True,
            #         trial = trial,
            #     )
            #     print "db created and now syning---------",new_client_id
            #     if new_client_id:
            #         new_client_id.sync_client()
            #     else:
            #         pass
        except:
