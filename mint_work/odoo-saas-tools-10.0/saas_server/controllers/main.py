...
class SaasServer(http.Controller):
    ...
    def new_database(self, **post):
        ...
        print "\n\n\n\ state in new_db controoller ------",state
...
#         addons = state.get('addons', ['hide_menu'])
        print 'Addons ' + 5 * '=', addons
        ...
        # """
        #     @shahbaz : code to check if client already exist
        # """
        # client = request.env['saas_server.client'].sudo().search(
        #     [('client_id','=',client_id)])
        # print "\n\n\n client new_db create controller-------------",client
        # if not client:
        #     client_data = {
        #         'name': new_db,
        #         'client_id': client_id,
        ...
        client = request.env['saas_server.client'].sudo().create(client_data)
        # ==========this default code ends here ==================
        print "\n\n\ after checking client  --------",client
        res = client.create_database(template_db, demo, lang)
        """
            
            @Pavan : added code for the client side install this addons
            'Hide menu' module install at the client side whenever client created
