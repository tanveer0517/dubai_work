...
# _API_URL = "http://www.marakezcloud.com/api/"
...
def setCity(_API_URL, id, name, code, state_id, _active):
    ...
    request_city = requests.post(_API_URL + "setCity", data=_data, headers=headers)
...
def setLocation(_API_URL, state_id, city_id, loc_id, loc_name , _active):
    ...
    req_loc = requests.post(_API_URL + "setLocation", data=_data, headers=headers)
...
def setCategory(_API_URL, id , name, group_id, app_img, img_name, allow_product, _active):
    ...
    req_cat = requests.post(_API_URL + "setCategory", data=_data, headers=headers)
    if req_cat.status_code == 200:
            return {"status_code":req_cat.status_code,
                    "category_id":req_cat.json().get("gomart_category_id"),
                    "json_dump":req_cat.json(), "data":_data}
    else:
        return {"status_code":req_cat.status_code, "data":_data}
...
def setSubCategory(_API_URL, id, cat_id, sub_cat_name, group_id, img_name, _active):
    ...
    req_subcat = requests.post(_API_URL + "setSubCategory", data=_data, headers=headers)
    if req_subcat.status_code == 200:
        ...
        return {"status_code":req_subcat.status_code, "data":_data}
...
def setUnit(_API_URL, id, name, _active):
    ...
    if req_unit.status_code == 200:
        ...
        return {"status_code":req_unit.status_code, "data":_data}
...
def setStore(_API_URL, id, name, chain_id, city_id, loc_id, stat_id, about, merchant_type, commission, start_dat, flag,
            store_type, address, email, lattitude, longitude, web, phone1, phone2, floor_flag, rating, _active):
    """
    Go Mart,Using POST method
    API 9 : setStore
    ...
    _data = {
            "client_key":_client_key,
            "erp_store_id":id or 1,
            "erp_store_name":name or '',
            "erp_store_chain_id":chain_id or 1,
            "erp_city_id":city_id or 1,
            "erp_location_id":loc_id or 1,
            "erp_state_id":stat_id or 1 ,
            "erp_about":about or '',
            "erp_merchant_type":merchant_type or 1,
            "erp_commission":commission or "2.5",
            "erp_start_date":start_dat or datetime.now().date(),
            "erp_chain_flag":flag or 1,
            "erp_primary_store":store_type or 1,
            "erp_address":address or '',
            "erp_store_mail":email or 'no@email.com',
            "erp_lattitude":lattitude or '00.0000',
            "erp_longitude":longitude or '00.0000',
            "erp_store_web":web or 'http://test.com',
            "erp_store_phone1":phone1 or '984612345',
            "erp_store_phone2":phone2 or '984612343',
            "erp_floor_plan_flag":floor_flag or 1,
            "erp_store_rating":rating or 5,
            "active":_active or 1
            }

    req_store = requests.post(_API_URL + "setStore", data=_data)
...
def setStoreAdmin(_API_URL, id, store_chain_id, admin_id, admin_name, login, pwd, email, mobile, dob, admin_type,
                  super_admin, chain_super_admin, gender):
    """
    Go Mart,Using POST method
    ...
    store_admin = requests.post(_API_URL + "setStoreAdmin", data=_data)
    if (store_admin.status_code == 200) and (store_admin.json().get("code") == 200):
          ...
          return {"data":_data, "status_code":store_admin.status_code}
...
def setProduct(_API_URL, id, name, cat_id, sub_cat_id, brand_id, family_id, small_description, description, _active):
    ...
    req_product = requests.post(_API_URL + "setProduct", data=_data)
...
def setProductRange(_API_URL, product_id, id, name, uom_id, range_img, barcode):
    """
    Go Mart,Using POST method
    ...
    @param erp_unit_id: Unit of Measurement
...
#     addedby
    ...
    req_range = requests.post(_API_URL + "setProductRange", data=_data)
    if req_range.status_code == 200:
        ...
        return{"data":_data, "status_code":req_range.status_code, "json_dump":req_range.json()}


def setStoreProduct(_API_URL, id, store_id, chain_id, product_id, cat_id, subcat_id, sku):
    """
    Go Mart,Using POST method
    ...
    req_store_product = requests.post(_API_URL + "setStoreProduct", data=_data)
    if req_store_product.status_code == 200:
        ...
        return {"data":_data, "status_code":req_store_product.status_code}


def setStoreProductRange(_API_URL, product_rang_id, rang_id, store_id, chain_id, product_id, mrp_price, prod_disc,
                         price_offer, qty, unlimited, pur_limit, out_stock, low_stock, barcod, added_by):
    """
    Go Mart,Using POST method
    ...
    req_store_prod_rang = requests.post(_API_URL + "setStoreProductRange", data=_data)
    if req_store_prod_rang.status_code == 200:
            return {
                    "status_code":req_store_prod_rang.status_code,
                    "store_product_range_id":req_store_prod_rang.json().get("gomart_store_range_id"),
                    "data":_data,
                    "json_dump":req_store_prod_rang.json()
                   }
    else:
        return {
                "data":_data, "status_code":req_store_prod_rang.status_code
                }


def setInventoryStock(_API_URL, store_rang_id, qty, unlimit, pur_limit, out_stock, low_stock):
    """
    Go Mart,Using POST method
...
def setStoreDetail(_API_URL, store_id, chain_id, city_id, state_id, location_id, store_name,
                   ...
                   about, onfloor, store_rate, _active):
    """
    Go Mart,Using POST method
    ...
    req_store_details = requests.post(_API_URL + "setStoreDetail", data=_data)
    if req_store_details.status_code == 200:
            return {
                    "status_code":req_store_details.status_code,
                    "store_chain_id":req_store_details.json().get("gomart_store_chain_id"),
                    "data":_data,
                    "json_dump":req_store_details.json()
                    }
    else:
        return {
                "data":_data, "status_code":req_store_details.status_code
                }
...
def setState(_API_URL, name, id, _active):
    ...
    req_state = requests.post(_API_URL + "setState", data=_data)
    if req_state.status_code == 200:
            return{"status_code":req_state.status_code,
                 "json_dump":req_state.json(),
                 "data":_data,
                 "state_id":req_state.json().get("gomart_state_id")}
    else:
        return {"status_code":req_state.status_code,
                 "json_dump":req_state.json(),
                 "data":_data}
...
def setStoreExtras(_API_URL, field_id, store_id, store_chain_id, order_prefix, order_amt, delivery_chrg,
                        ...
                        sat_flag, sat_st, sat_et, sun_flag, sun_st, sun_et, pay_options):
    """
    Go Mart,Using POST method
    ...
    store_extras = requests.post(_API_URL + "setStoreExtras", data=_data)
    if store_extras.status_code == 200: 
        return {
                "status_code":store_extras.status_code,
                "store_field_id":store_extras.json().get('gomart_store_field_id'),
                "data":_data,
                "json_dump":store_extras.json()
                }
    else: 
        return {
                "data":_data, "status_code":store_extras.status_code
                }


def setDeliveryRegion(_API_URL, region_id, chain_id, location_id):
    """
    Go Mart,Using POST method
    ...
    delivery_region = requests.post(_API_URL + "setDeliveryRegion", data=_data)
    if delivery_region.status_code == 200: 
        return {
                "status_code":delivery_region.status_code,
                "delivery_region_id":delivery_region.json().get('gomart_delivery_region_id'),
                "data":_data,
                }
    else: 
        return {
...
def setGroup(_API_URL, id, name):
    ...
    group_request = requests.post(_API_URL + "setGroup", data=_data)
    if group_request.status_code == 200: 
        return {
                "status_code":group_request.status_code,
                "group_id":group_request.json().get('gomart_group_id'),
                "data":_data,
                "json_dump":group_request.json()
                }
    else: 
        return {
                "data":_data,
...
def setGroupFields(_API_URL, group_id, field_id, label, field_type, data_type, no_char,
    ...
    payload = "client_key=" + str(_client_key) + "&erp_group_id=" + str(group_id) + "&erp_field_id=" + str(field_id) + "&field_label=" + str(label) + "&field_type=" + str(field_type) + "&field_data_type=" + str(data_type) + "&no_of_chars=" + str(no_char) + "&decimal_places=" + str(decimal_place) + "&interval_allowed=" + str(allow) + "&measure_unit=" + str(unit) + "&currency_type=" + str(curr_type) + "&fixed_field_type=" + str(fix_field_type) 
    if isinstance(values, list):
        if len(values) <= 1:
            data = "&fixed_values%5B%5D="
            data1 = "".join(values)
            payload = payload + data + data1
        else:
            data = "&fixed_values%5B%5D="
            data1 = "&fixed_values%5B%5D=".join(values)
            payload = payload + data + data1
    else:
        pass
    _data = {
    ...
    group_fields = requests.post(_API_URL + "setGroupFields", data=payload, headers=headers)
    if group_fields.status_code == 200: 
        return {"status_code":group_fields.status_code,
                "field_id":group_fields.json().get('gomart_field_id'),
                "data":_data,
                "json_dump":group_fields.json(),
                "payload":payload
               }
    else: 
        return {"status_code":group_fields.status_code,
...
def setOrderStatus(_API_URL, id, status, store_id, chain_id):
    _data = {
            "client_key":_client_key,
            "erp_order_id":group_id,
            "erp_order_status":field_id,
            "erp_store_id":store_id,
            "erp_store_chain_id":label
            
            }
    group_fields_API = requests.post(_API_URL + "setOrderStatus", data=_data)
    if group_fields_API.status_code == 200: 
        return  {"order_id":group_fields_API.json().get("gomart_order_id"),
                 ...
                 "data":_data,
                 "status_code":group_fields_API.status_code}
    else:
        return {"status_code":group_fields_API.status_code,
                "json_dump":group_fields_API.json(),
                "data":_data}
