...
class PosSaleReport(models.Model):
    ...
    def _sale_order_select(self):
        select = """SELECT min(sol.id)*-1 AS id,
            ...
            GROUP BY so.date_order, so.partner_id, sol.product_id, 
            pp.product_tmpl_id,
    ...
    def _pos_order_select(self):
        select = """SELECT min(pol.id) AS id,
            ...
            po.partner_id AS customer,
            pol.product_id AS product_id,
            pt.categ_id AS product_categ_id,
            pt.pos_categ_id AS pos_categ_id,
            pp.product_tmpl_id AS product_tmpl_id,
            po.company_id AS company_id,
            'POS Orders' AS origin,
            sum(pol.qty) AS qty
            ...
            GROUP BY po.date_order, po.partner_id, pol.product_id, 
            pp.product_tmpl_id,
