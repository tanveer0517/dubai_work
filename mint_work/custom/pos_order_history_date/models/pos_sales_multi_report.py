...
class PosSalesReportDateRange(models.TransientModel):
    _name = 'pos.sales.report.date'
    ...
    #  which will be in range betwwen the dates selected.
    @api.multi
    def sales_order_report_date(self):
        self.ensure_one()
        data = {
            'ids': self.id,
        ...
        if result:
            ...
            raise UserError(
                _('There is no Record Between the selected Date Range.'))

        return self.env['report'].get_action(self,
                                             'pos_order_history_date.report_sale_orders_date', data=data)


class ReportPOSSaleOrderDateRangeMulti(models.AbstractModel):

    _name = 'report.pos_order_history_date.report_sale_orders_date'

    @api.multi
    def render_html(self, docids, data=None):
        return self.env['report'].render('pos_order_history_date.report_sale_orders_date', dict(data or {}))
