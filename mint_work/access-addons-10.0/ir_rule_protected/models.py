...
class IRRule(models.Model):
    ...
    protected = fields.Boolean('Protected',
                               help='Make rule editable only for superuser')
    ...
    def check_restricted(self):
        ...
        for r in self:
            if r.protected:
                raise exceptions.Warning(_("The Rule is protected. "
                                           "You don't have access for "
                                           "this operation"))
    ...
    def button_uninstall(self):
        for r in self:
            if r.name == MODULE_NAME and self.env.uid != SUPERUSER_ID:
                raise exceptions.Warning(_("Only admin can "
