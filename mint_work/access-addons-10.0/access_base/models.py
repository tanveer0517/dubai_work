...
from odoo.addons.base.res.res_users import name_boolean_group, \
    name_selection_groups


class GroupsView(models.Model):
    _inherit = 'res.groups'
    is_custom_group = fields.Boolean("Custom Group", help="show group at "
                                                          "the top of Access "
                                                          "Rights tab in user "
                                                          "form")
    ...
    def _update_user_groups_view(self):
        # call super to make module compatible with other modules (e.g.
        # access_restricted)
        ...
        if view and view.exists() and view._name == 'ir.ui.view':
            ...
            xml3 = []
            xml3.append(E.separator(string=_('Custom User Groups'),
                                    colspan="4"))

            for app, kind, gs in self.get_groups_by_application():
                xml = None
                custom = False
                if kind == 'selection' and any([g.is_custom_group for g in gs]) \
                ...
                if kind == 'selection':
                    ...
                    if not custom:
                        xml.append(E.separator(string=app_name, colspan="4",
                                               **attrs))
                    for g in gs:
                        field_name = name_boolean_group(g.id)
                        if g == group_no_one:
                            # make the group_no_one invisible in the form view
                            xml.append(E.field(name=field_name, invisible="1",
                                               **attrs))
                        else:
                            xml.append(E.field(name=field_name, **attrs))

            xml2.append({'class': "o_label_nowrap"})
            xml = E.field(E.group(*(xml3), col="2"), E.group(*(xml2), col="4"),
                          E.group(*(xml1), col="2"), name="groups_id",
                          position="replace")
