...
class BaseLimitRecordsNumber(models.Model):
    ...
    action_rule_id = fields.Many2one('base.action.rule', 'Base Action Rule',
    ...
    def create(self, vals):
        ...
        server_actions_ids = [(4, self.env.ref(
            'access_limit_records_number.action_check_records_number').id,
                               False)]
    ...
    def verify_table(self):
        ...
        for rule in self.search([('model_id.model', '=', model_name)]):
            records_count = self.env[model_name].\
                search_count(safe_eval(rule.domain))
            if records_count > rule.max_records:
                raise exceptions.Warning(
                    _('Maximimum allowed records in table "%(model_name)s" '
                      'is %(max_records)s, while after this update you would '
                      'have %(records_count)s') %
                    {'model_name': rule.model_id.name, 'max_records':
                        rule.max_records, 'records_count': records_count})
